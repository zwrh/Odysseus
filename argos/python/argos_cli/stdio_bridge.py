...
Provides a proper MCP Server that forwards all requests to Argos's StreamableHTTP endpoint.
Uses the MCP SDK's stdio transport and Pydantic serialization - no manual JSON-RPC handling.

Includes a ReconnectingBackend that automatically reconnects to the Argos server on
...
class ReconnectingBackend:
    """
    Manages a connection to the Argos StreamableHTTP backend with automatic reconnection.
...
class ArgosStdioBridge:
    """
    MCP Server that bridges stdio to Argos's StreamableHTTP endpoint.

    Acts as a transparent proxy - forwards all MCP requests to the Argos backend
    ...
    def __init__(self, port: int):
        ...
        Args:
            port: Argos server port to connect to
        """
        self.port = port
        self.url = f"http://localhost:{port}/mcp/message"
        self.server = Server("Argos")
    ...
    def _register_handlers(self):
        """Register MCP protocol handlers that forward to Argos backend."""

        @self.server.list_tools()
        async def list_tools() -> list[Tool]:
            """Forward list_tools request to Argos backend."""
            if not self.backend:
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent | ImageContent | EmbeddedResource]:
            """Forward call_tool request to Argos backend."""
            if not self.backend:
                raise RuntimeError("Backend not initialized")
        ...
        async def list_resources() -> list[Resource]:
            """Forward list_resources request to Argos backend."""
            if not self.backend:
                raise RuntimeError("Backend not initialized")
        ...
        async def read_resource(uri: str) -> str | bytes:
            """Forward read_resource request to Argos backend."""
            if not self.backend:
                raise RuntimeError("Backend not initialized")
        ...
        async def list_prompts() -> list[Prompt]:
            """Forward list_prompts request to Argos backend."""
            if not self.backend:
                raise RuntimeError("Backend not initialized")
    ...
    async def run(self):
        ...
        Connects to Argos backend via ReconnectingBackend, then exposes
        the MCP server via stdio transport.
        """
        print(f"Connecting to Argos backend at {self.url}...", file=sys.stderr)
