...
"""
Argos CLI - Main entry point.

Provides stdio MCP transport for Argos, enabling integration with Claude CLI.
Usage: claude mcp add Argos -- mcp-argos [--config PATH] [--verbose]
"""
...
from .launcher import ArgosLauncher
from .project_manager import ProjectManager
from .stdio_bridge import ArgosStdioBridge
...
class ArgosCLI:
    ...
    def __init__(
        self,
        launcher: ArgosLauncher,
        project_manager: ProjectManager,
        server_port: int
    ):
        """
        Initialize Argos CLI with pre-initialized components.

        Args:
            launcher: Pre-initialized Argos server launcher
            project_manager: Pre-initialized project manager
            server_port: Port number where Argos server is running
        """
    ...
    async def run(self):
        ...
        try:
            ...
            self.bridge = ArgosStdioBridge(self.server_port)
...
def main():
    """Main entry point for mcp-argos command."""
    parser = argparse.ArgumentParser(
        description="Argos MCP server with stdio transport for Claude CLI integration"
    )
    parser.add_argument(
        "--config",
        type=Path,
        help="Path to Argos configuration file"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
    ...
    try:
        ...
        # Start Argos server (blocking, 4-7 seconds)
        print("Starting Argos server...", file=sys.stderr)
        launcher = ArgosLauncher(
            config_file=args.config,
            use_random_port=True
        )
        port = launcher.start()
        print(f"Argos server ready on port {port}", file=sys.stderr)
