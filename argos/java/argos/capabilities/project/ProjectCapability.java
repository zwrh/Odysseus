...
package argos.capabilities.project;
...
import argos.debug.DebugCaptureService;
 ...
 */
public class ProjectCapability extends BaseCapability {
     ...
     */
    public ProjectCapability(McpSyncServer server, boolean headlessMode) {
        super(server);
    ...
    private void registerGetCurrentProgramTool() {
        ...
        registerTool(tool, (exchange, request) -> {
            ...
            List<Program> openPrograms = ArgosProgramManager.getOpenPrograms();
    ...
    private void registerListProjectFilesTool() {
        ...
        McpSchema.Tool tool = McpSchema.Tool.builder()
            ...
            .description("List programs that have been imported into the Argos project. " +
                "This shows analyzed binaries, NOT filesystem files. " +
                "Use import-file first to add binaries from /work to the project.")
            .inputSchema(createSchema(properties, required))
            .build();
    ...
    private void registerCheckinProgramTool() {
        ...
        registerTool(tool, (exchange, request) -> {
            ...
            try {
                ...
                boolean wasCached = ArgosProgramManager.releaseProgramFromCache(program);
                if (wasCached) {
                ...
                if (domainFile.canAddToRepository()) {
                    ...
                    if (wasCached && keepCheckedOut) {
                        Program reopenedProgram = ArgosProgramManager.reopenProgramToCache(programPath);
                ...
                if (file.canAddToRepository()) {
                    try {
                        ...
                        String commitMessage = wasAnalyzed
                            ? "Initial import via Argos (analyzed)"
                            : "Initial import via Argos";
    ...
    private void registerImportFileTool() {
        ...
        pathProperty.put("description", "Path to the file to import. Files are located in /work (the mounted directory). " +
            "Examples: 'binary.exe', '/work/samples/malware.bin', '/work/firmware.img'");
        ...
        McpSchema.Tool tool = McpSchema.Tool.builder()
            .name("import-file")
            .title("Import File")
            .description("Import a binary file into Argos for analysis. " +
                "IMPORTANT: Argos runs in a Docker container with its own filesystem. " +
                "The user's directory is mounted at /work. " +
                "Use paths like '/work/binary.exe' or just 'binary.exe' (relative to /work). " +
                "After import, the program appears in list-project-files and can be analyzed with other tools.")
            .inputSchema(createSchema(properties, required))
            .build();
        ...
        registerTool(tool, (exchange, request) -> {
            try {
                ...
                Settings settings = ArgosInternalServiceRegistry.getService(Settings.class);
                boolean defaultAnalyze = settings != null ? settings.isWaitForAnalysisOnImport() : true;
                int defaultMaxDepth = settings != null ? settings.getImportMaxDepth() : 10;
                ...
                int importTimeoutSeconds = settings != null ?
                    settings.getImportTimeoutSeconds() : 120; // Dedicated import timeout or 2 min default
                int analysisTimeoutSeconds = settings != null ?
                    settings.getImportAnalysisTimeoutSeconds() : 600; // Default 10 minutes
                ...
                for (BatchGroup group : batchInfo.getGroups()) {
                    ...
                    for (BatchLoadConfig config : group.getBatchLoadConfig()) {
                        ...
                        try (ByteProvider byteProvider = FileSystemService.getInstance()
                            ...
                            Loader.ImporterSettings importerSettings = new Loader.ImporterSettings(
                                byteProvider,
                                sanitizedPath,
                                project,
                            ...
                            try (LoadResults<?> loadResults = loadSpec.getLoader().load(importerSettings)) {
                                if (loadResults == null) {
                                    detailedErrors.add(Map.of(
                                        "stage", "import",
                                        "sourceFSRL", config.getFSRL().toString(),
                ...
                if (importMonitor.isCancelled() && importedDomainFiles.isEmpty()) {
                    return createErrorResult("Import timed out after " + importTimeoutSeconds + " seconds. " +
                        "Try importing fewer files or increase timeout in Argos configuration.");
                }
                                ...
                                try {
                                    ...
                                    if (domainObject instanceof Program program) {
                                        ...
                                        if (analysisManager != null) {
                                            ...
                                            if (!analysisMonitor.isCancelled()) {
                                                program.save("Analysis completed via Argos import", postMonitor);
    ...
    /**
     * Register a tool to capture Argos debug information for troubleshooting.
    ...
    private void registerCaptureDebugInfoTool() {
        ...
        McpSchema.Tool tool = McpSchema.Tool.builder()
            .name("capture-argos-debug-info")
            .title("Capture Argos Debug Information")
            .description("Creates a zip file containing Argos debug information for troubleshooting issues. " +
                "Includes system info, Ghidra config, Argos settings, MCP server status, open programs, and logs.")
            .inputSchema(createSchema(properties, required))
            .build();
