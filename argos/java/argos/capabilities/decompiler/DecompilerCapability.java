...
package argos.capabilities.decompiler;
...
import argos.util.DataTypeParserUtil;
import argos.util.DecompilationContextUtil;
import argos.util.DecompilationDiffUtil;
import argos.util.DebugLogger;
import argos.util.ArgosInternalServiceRegistry;

/**
 * Tool provider for function decompilation operations.
 */
public class DecompilerCapability extends BaseCapability {
     ...
     */
    public DecompilerCapability(McpSyncServer server) {
        super(server);
    }
    ...
    public void programClosed(Program program) {
        ...
        if (removed > 0) {
            logInfo("DecompilerCapability: Cleared " + removed +
    ...
    private TaskMonitor createTimeoutMonitor() {
        ...
        int timeoutSeconds = settings.getDecompilerTimeoutSeconds();
    ...
    private int getTimeoutSeconds() {
        return ArgosInternalServiceRegistry.getService(Settings.class).getDecompilerTimeoutSeconds();
    }
                ...
                while (functions.hasNext() && results.size() < maxResults) {
                    ...
                    try {
                        ...
                        if (isTimedOut(functionTimeoutMonitor)) {
                            Msg.warn(DecompilerCapability.class, toolName + ": Decompilation timed out for function " +
