...
package argos.capabilities.datatypes;
 ...
 */
public class DataTypeCapability extends BaseCapability {
    /**
     * Constructor
     * @param server The MCP server
     */
    public DataTypeCapability(McpSyncServer server) {
        super(server);
    }
    ...
    private void registerGetDataTypeArchivesTool() {
        ...
        registerTool(tool, (exchange, request) -> {
            ...
            // Try to add standalone data type managers if ArgosPlugin is available
            // This is optional - if not available, we still have built-in and program types
            argos.plugin.ArgosPlugin plugin = ArgosInternalServiceRegistry.getService(argos.plugin.ArgosPlugin.class);
            if (plugin != null) {
