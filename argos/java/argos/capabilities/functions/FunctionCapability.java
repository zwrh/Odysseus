...
package argos.capabilities.functions;
...
import argos.util.SymbolUtil;

/**
 * Tool provider for function-related operations.
 */
public class FunctionCapability extends BaseCapability {
     ...
     */
    public FunctionCapability(McpSyncServer server) {
        super(server);
    }
    ...
    public void programClosed(Program program) {
        ...
        if (removedSimilarity > 0 || removedFunctionInfo > 0) {
            logInfo("FunctionCapability: Cleared " + removedSimilarity +
    ...
    private List<Map<String, Object>> getOrBuildFunctionInfoCache(Program program, boolean filterDefaultNames) {
        ...
        if (cached != null && !cached.isExpired() && cached.programModificationNumber() == currentModNumber) {
            logInfo("FunctionCapability: Using cached function info for " + programPath);
        ...
        synchronized (functionInfoCache) {
            ...
            logInfo("FunctionCapability: Building function info cache for " + programPath);
            ...
            while (functions.hasNext()) {
                ...
                if (processed % 100 == 0 && monitor.isCancelled()) {
                    logInfo("FunctionCapability: Cache build timed out after " + processed + " functions");
                    break;
                }
            ...
            if (elapsed > SLOW_SEARCH_THRESHOLD_MS) {
                logInfo("FunctionCapability: Building function info cache took " +
    ...
    private void evictFunctionInfoCacheIfNeeded() {
        ...
        while (functionInfoCache.size() >= MAX_FUNCTION_INFO_CACHE_ENTRIES) {
            ...
            if (oldest != null) {
                ...
                logInfo("FunctionCapability: Evicted function info cache entry for: " + oldest.programPath());
            } else {
                break;
            }
        }
