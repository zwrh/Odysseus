...
import argos.util.ProgramLookupUtil;
...
/**
 * Base implementation of the Capability interface.
...
public abstract class BaseCapability implements Capability {
    ...
    protected final List<Tool> registeredCapabilities = new ArrayList<>();

    /**
     * Constructor
     * @param server The MCP server to register tools with
     */
    public BaseCapability(McpSyncServer server) {
        this.server = server;
    }
    ...
    protected void registerTool(Tool tool, java.util.function.BiFunction<io.modelcontextprotocol.server.McpSyncServerExchange, CallToolRequest, McpSchema.CallToolResult> handler) {
        ...
        java.util.function.BiFunction<io.modelcontextprotocol.server.McpSyncServerExchange, CallToolRequest, McpSchema.CallToolResult> safeHandler =
            (exchange, request) -> {
                String requestId = ArgosToolLogger.generateRequestId();
                long startTime = System.currentTimeMillis();

                // Log request
                ArgosToolLogger.logRequest(tool.name(), requestId, request.arguments());

                // Log to Ghidra's application log for correlation
                Msg.debug(BaseCapability.class, String.format("[Argos:%s] Tool call: %s",
                    requestId, tool.name()));
                ...
                try {
                    ...
                    ArgosToolLogger.logResponse(tool.name(), requestId, durationMs,
                        result != null && result.isError(), result);

                    // Log completion to Ghidra's application log
                    Msg.debug(BaseCapability.class, String.format("[Argos:%s] Tool completed: %s (%dms)",
                        requestId, tool.name(), durationMs));

                    return result;
                } catch (IllegalArgumentException e) {
                    long durationMs = System.currentTimeMillis() - startTime;
                    ArgosToolLogger.logError(tool.name(), requestId, durationMs, e.getMessage());
                    Msg.debug(BaseCapability.class, String.format("[Argos:%s] Tool error: %s - %s (%dms)",
                    ...
                    long durationMs = System.currentTimeMillis() - startTime;
                    ArgosToolLogger.logError(tool.name(), requestId, durationMs, e.getMessage());
                    Msg.error(BaseCapability.class, String.format("[Argos:%s] Tool failed: %s (%dms)",
