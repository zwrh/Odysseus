...
import argos.plugin.config.ConfigurationBackend;
import argos.plugin.config.ConfigurationBackendListener;
import argos.plugin.config.FileBackend;
import argos.plugin.config.InMemoryBackend;
import argos.plugin.config.ToolOptionsBackend;

/**
 * Configuration manager for the Argos plugin.
 ...
 */
public class Settings implements ConfigurationBackendListener {
    // Configuration option categories
    public static final String SERVER_OPTIONS = "Argos Server Options";
     ...
     */
    public Settings(PluginTool tool) {
     ...
     */
    protected Settings(ConfigurationBackend backend) {
    ...
    private void registerOptionsWithGhidra(ToolOptionsBackend toolBackend) {
        HelpLocation help = new HelpLocation("Argos", "Configuration");

        var toolOptions = toolBackend.getToolOptions();

        toolOptions.registerOption(SERVER_PORT, DEFAULT_PORT, help,
            "Port number for the Argos MCP server");
        toolOptions.registerOption(SERVER_HOST, DEFAULT_HOST, help,
            "Host interface for the Argos MCP server (127.0.0.1 for localhost only, 0.0.0.0 for all interfaces)");
        toolOptions.registerOption(SERVER_ENABLED, DEFAULT_SERVER_ENABLED, help,
            "Whether the Argos MCP server is enabled");
        ...
        toolOptions.registerOption(REQUEST_LOGGING_ENABLED, DEFAULT_REQUEST_LOGGING_ENABLED, help,
            "Enable detailed logging of MCP tool requests and responses to argos-tools.log");
    ...
    protected void loadOptions() {
        ...
        Msg.debug(this, "Loaded Argos configuration settings");
    }
    ...
    /**
     * Generate a default API key with Argos-UUID format
     * @return A new API key in the format "Argos-{uuid}"
     */
    private String generateDefaultApiKey() {
        return "Argos-" + UUID.randomUUID().toString();
    }
