 ...
 */
public class ArgosProgramManager {
    ...
    public static List<Program> getOpenPrograms() {
        ...
        if (project == null) {
            Msg.debug(ArgosProgramManager.class, "No active project found");
            return openPrograms;
        }
        ...
        if (toolManager != null) {
            ...
            Msg.debug(ArgosProgramManager.class, "Found " + runningTools.length + " running tools");

            for (PluginTool tool : runningTools) {
                ProgramManager programManager = tool.getService(ProgramManager.class);
                if (programManager != null) {
                    Program[] programs = programManager.getAllOpenPrograms();
                    Msg.debug(ArgosProgramManager.class, "Tool " + tool.getName() + " has " + programs.length + " open programs");
                    for (Program program : programs) {
                        if (!openPrograms.contains(program)) {
                            openPrograms.add(program);
                            Msg.debug(ArgosProgramManager.class, "Added program: " + program.getName() + " with domain path: " + program.getDomainFile().getPathname());
                        }
                    }
                } else {
                    Msg.debug(ArgosProgramManager.class, "Tool " + tool.getName() + " has no ProgramManager service");
                }
            }
        } else {
            Msg.debug(ArgosProgramManager.class, "No tool manager found");
        }

        // If no tools were found (common in test environments),
        // try to get programs directly from the ArgosPlugin's tool
        if (openPrograms.isEmpty()) {
            Msg.debug(ArgosProgramManager.class, "No programs found via ToolManager, trying ArgosPlugin tool");
            ArgosPlugin argosPlugin = ArgosInternalServiceRegistry.getService(ArgosPlugin.class);
            if (argosPlugin != null && argosPlugin.getTool() != null) {
                PluginTool tool = argosPlugin.getTool();
                ProgramManager programManager = tool.getService(ProgramManager.class);
                if (programManager != null) {
                    Program[] programs = programManager.getAllOpenPrograms();
                    Msg.debug(ArgosProgramManager.class, "ArgosPlugin tool has " + programs.length + " open programs");
                    for (Program program : programs) {
                        if (!openPrograms.contains(program)) {
                            openPrograms.add(program);
                            Msg.debug(ArgosProgramManager.class, "Added program from ArgosPlugin: " + program.getName() + " with domain path: " + program.getDomainFile().getPathname());
                        }
                    }
                } else {
                    Msg.debug(ArgosProgramManager.class, "ArgosPlugin tool has no ProgramManager service");
                }
            } else {
                Msg.debug(ArgosProgramManager.class, "ArgosPlugin not found or has no tool");
            }
        }

        Msg.debug(ArgosProgramManager.class, "Total open programs found: " + openPrograms.size());
        return openPrograms;
    }
    ...
    public static void registerProgram(Program program) {
        if (program != null && !program.isClosed()) {
            ...
            Msg.debug(ArgosProgramManager.class, "Registered program: " + programPath);
        }
    }
    ...
    public static void unregisterProgram(Program program) {
        if (program != null) {
            ...
            Msg.debug(ArgosProgramManager.class, "Unregistered program: " + programPath);
        }
    }
    ...
    public static void programClosed(Program program) {
        if (program != null) {
            ...
            registeredPrograms.remove(programPath);
            programCache.remove(programPath);
            Msg.debug(ArgosProgramManager.class, "Program closed, cleared cache: " + programPath);
        }
    }
    ...
    public static void programOpened(Program program) {
        if (program != null && !program.isClosed()) {
            ...
            programCache.remove(programPath);
            Msg.debug(ArgosProgramManager.class, "Program opened, cleared stale cache: " + programPath);
        }
    }
    ...
    private static void ensureCheckedOut(Program program, String programPath) {
        ...
        Msg.debug(ArgosProgramManager.class,
    ...
    public static Program getProgramByPath(String programPath) {
        ...
        Msg.debug(ArgosProgramManager.class, "Looking for program with path: " + programPath);
        ...
        if (registeredPrograms.containsKey(programPath)) {
            ...
            if (!registeredProgram.isClosed()) {
                Msg.debug(ArgosProgramManager.class, "Found program in registry: " + programPath);
        ...
        if (programCache.containsKey(programPath)) {
            ...
            if (!cachedProgram.isClosed()) {
                Msg.debug(ArgosProgramManager.class, "Found program in cache: " + programPath);
        ...
        Msg.debug(ArgosProgramManager.class, "Checking " + openPrograms.size() + " open programs");

        for (Program program : openPrograms) {
            // Check the Ghidra project path first (most common case)
            String domainPath = program.getDomainFile().getPathname();
            Msg.debug(ArgosProgramManager.class, "Comparing '" + programPath + "' with domain path '" + domainPath + "'");
            if (domainPath.equals(programPath)) {
                ...
                Msg.debug(ArgosProgramManager.class, "Found program by domain path: " + programPath);
            ...
            Msg.debug(ArgosProgramManager.class, "Also checking executable path '" + executablePath + "' and name '" + programName + "'");
            if (executablePath.equals(programPath) || programName.equals(programPath)) {
                // Use canonical domain path as cache key for consistency
                String canonicalPath = getCanonicalProgramPath(program);
                programCache.put(canonicalPath, program);
                Msg.debug(ArgosProgramManager.class, "Found program by executable path or name: " + programPath);
                ensureCheckedOut(program, programPath);
                return program;
            }
        }
        ...
        Project project = AppInfo.getActiveProject();
        if (project == null) {
            Msg.warn(ArgosProgramManager.class, "No active project");
            return null;
        }
        ...
        if (domainFile == null) {
            Msg.warn(ArgosProgramManager.class, "Could not find program: " + programPath);
            return null;
        }
